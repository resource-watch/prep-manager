    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        case "develop":
          ...
          break
        case "preproduction":
          sh("docker -H :2375 build --build-arg secretKey=${secretKey} --build-arg apiEnv=production,preproduction --build-arg apiUrl=https://preproduction.prepdata.org/api --build-arg callbackUrl=https://preproduction.prepdata.org/auth -t ${imageTag} .")
          break
        case "master":
    ...
    stage ("Deploy Application") {
      switch ("${env.BRANCH_NAME}") {
        ...
        case "develop":
          ...
          def service = sh([returnStdout: true, script: "kubectl get deploy ${appName}-staging --namespace=prep || echo NotFound"]).trim()
        ...
        case "preproduction":
          sh("echo Deploying to STAGING cluster")
          sh("kubectl config use-context gke_${GCLOUD_PROJECT}_${GCLOUD_GCE_ZONE}_${KUBE_PROD_CLUSTER}")
          def service = sh([returnStdout: true, script: "kubectl get deploy ${appName}-preproduction --namespace=prep || echo NotFound"]).trim()
          if ((service && service.indexOf("NotFound") > -1) || (forceCompleteDeploy)){
            sh("sed -i -e 's/{name}/${appName}/g' k8s/preproduction/*.yaml")
            sh("kubectl apply -f k8s/preproduction/")
          }
          sh("kubectl set image deployment ${appName}-preproduction ${appName}-preproduction=${imageTag} --namespace=prep --record")
          break
